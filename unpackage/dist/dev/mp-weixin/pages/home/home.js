...
//
//
var _default = { data: function data() {return { msgList: [], flowName: ['已提交', '正在处理', '已完成'], counts: [96, 78, 18], modalName: null, gridCol: 5, gridBorder: false, menuBorder: false, menuArrow: false, menuCard: false, skin: false, loginStat: true, listTouchStart: 0, listTouchDirection: null, type: '0' };}, methods: { SetType: function SetType(e) {this.type = e.detail.value;this.initPages();}, doneTimeStr: function doneTimeStr(timeStr) {return _utils.default.get_time_str(timeStr);}, queryByFileno: function queryByFileno(e) {console.log("queryByFileno:" + JSON.stringify(e));uni.navigateTo({ url: '/pages/undo/flow-timeline' });}, updated: function updated() {this.initPages();console.log("HOME==========updated:" + this.loginStat);}, loginApp: function loginApp(e) {uni.navigateTo({ url: '/pages/login/login' });}, initPages: function initPages() {console.log("HOME==========initPages:" + this.loginStat);var _this = this; //初始化页面消息
      var data = {};data.userId = '15071177366';data.type = _this.type;_this.commRequest('/app/getMsgList', data, function (res) {console.log("/app/getMsgList success in home:" + res);_this.msgList = res.list;});}, queryBynum: function queryBynum(index) {var _this = this;uni.navigateTo({ url: '/pages/home/allthings?index=' + index + '&type=' + _this.type });} },
